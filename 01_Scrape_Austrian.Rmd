...
scrape_proposal <- function(url) { # url = proposal_link
  ...
  # read in proposalpage from url
  proposalpage <- read_html(url)
  
  # if else status BR & NR for different legislative periods and website format (bill_link, resolution_BR)
  if(period == "XXV" | period == "XXIV" | period == "XXIII" & status == "beschlossen" & type == "A" | type == "RV") {
    ...
    # scrape bill_link (??) für welche LP??? else if oder else??
    bill_link <- proposalpage %>%
      html_node(., "div.floatLeft p")
  }
  
  # resolution NR (same for all LP) (dafür/dagegen) --> tibble, aber wie ????!?!!!
  resolution_NR <- proposalpage %>%
    html_node(., "div.floatLeft p") %>%
    html_text(.)
}
